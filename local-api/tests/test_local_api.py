    ...
    def test_live_agent_test_creates_encrypted_trace_and_safe_evidence(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertEqual(payload["agent_under_test"]["execution_mode"], "real_agent")
                self.assertEqual(payload["agent_under_test"]["source_file"], "examples/claims-triage-python/claims_triage.py")
                self.assertIn("Capsule.wrap_tool", payload["agent_under_test"]["instrumentation"])
